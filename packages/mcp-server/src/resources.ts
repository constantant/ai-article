...
import {
  appProfileJsonSchema,
  articleDraftInputJsonSchema,
  articleJsonSchema,
} from '@org/schema';
...
export function registerResources(server: McpServer, client: RestClient): void {
  server.registerResource(
    ...
    'schema://article',
    {
      ...
      description:
        'The full stored Article shape (id, status, timestamps, blocks, ...). Read-only reference.',
      mimeType: 'application/json',
    },
  ...
  server.registerResource(
    ...
    'schema://article-draft',
    {
      ...
      description:
        'The shape expected by create_article/validate_article — no id/status/timestamps yet.',
      mimeType: 'application/json',
    },
  ...
  server.registerResource(
    ...
    new ResourceTemplate('profile://{appId}', { list: undefined }),
    {
      ...
      description:
        'Voice, allowed block types, content conventions, and design tokens for one app.',
      mimeType: 'application/json',
    },
    async (uri, variables) =>
      jsonContents(uri, await client.getAppProfile(String(variables['appId']))),
  );
  ...
  server.registerResource(
    ...
    new ResourceTemplate('example://{appId}/{id}', { list: undefined }),
    {
      ...
      description:
        "Fetch one of an app's existing articles by id to see its structure and tone in practice.",
      mimeType: 'application/json',
    },
    async (uri, variables) =>
      jsonContents(
